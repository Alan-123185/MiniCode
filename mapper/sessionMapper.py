...
class sessionMapper:
    ...
    def query_session_by_user_id(self, user_id: str) -> list[dict] | None:
        return  self.db.fetch_all("SELECT * FROM session WHERE user_id = ?", (user_id,))

    def query_session_by_session_id(self, session_id: str) -> dict | None:
        return self.db.fetch_one("SELECT * FROM session WHERE id = ?", (session_id,))

    def update_workplace(self, session_id: str, workplace: str) -> None:
        self.db.execute(
